                    ...
                    <div class="control-group">
                        <select class="control" id="locale-switcher" onChange="window.location.href = this.value">
                            @foreach (core()->getAllLocales() as $localeModel)
                                ...
                                <option
                                    value="{{ route('blog.admin.tag.edit', $tag->id) . '?locale=' . $localeModel->code }}" {{ ($localeModel->code) == $locale ? 'selected' : '' }}>
                                    {{ $localeModel->name }}
                                </option>
                    ...
                    <accordian :title="'{{ __('blog::app.general.general') }}'" :active="true">
                        <div slot="body">
                            <div class="control-group" :class="[errors.has('{{$locale}}[name]') ? 'has-error' : '']">
                                <label for="{{$locale}}[name]" class="required">{{ __('blog::app.tag.name') }}</label>
                                <input type="text" class="control"
                                       id="{{$locale}}[name]"
                                       name="{{$locale}}[name]"
                                       v-validate="'required'"
                                       value="{{ old($locale)['name'] ?? ($tag->translate($locale)['name'] ?? '') }}"
                            ...
                            <div class="control-group" :class="[errors.has('channels[]') ? 'has-error' : '']">
                                ...
                                <?php $selectedOptionIds = old('inventory_sources') ?: $tag->channels->pluck('id')->toArray() ?>
                        ...
                        <div slot="body">
                            <div class="control-group" :class="[errors.has('{{$locale}}[url_key]') ? 'has-error' : '']">
                                ...
                                <input type="text" class="control" name="{{$locale}}[url_key]" v-validate="'required'"
                                       value="{{ old($locale)['url_key'] ?? ($tag->translate($locale)['url_key'] ?? '') }}"
