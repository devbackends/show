...
@section('page_title')
    {{ __('blog::app.post_category.edit-category') }}
@stop
@section('content')
    <div class="content">
            ...
            <div class="page-header">
                    ...
                    <div class="control-group">
                        <select class="control" id="locale-switcher" onChange="window.location.href = this.value">
                            @foreach (core()->getAllLocales() as $localeModel)
                                ...
                                <option
                                    value="{{ route('blog.admin.post_category.edit', $category->id) . '?locale=' . $localeModel->code }}" {{ ($localeModel->code) == $locale ? 'selected' : '' }}>
                                    {{ $localeModel->name }}
                                </option>
                ...
                <div class="page-action">
                    <button type="submit" id="save_page_btn" class="btn btn-primary">
                        {{ __('blog::app.post_category.save-category') }}
                    </button>
                </div>
            </div>
                    ...
                    <accordian :title="'{{ __('blog::app.general.general') }}'" :active="true">
                        <div slot="body">
                            <div class="control-group" :class="[errors.has('{{$locale}}[name]') ? 'has-error' : '']">
                                <label for="{{$locale}}[name]" class="required">{{ __('blog::app.post_category.name') }}</label>
                                <input type="text" class="control"
                                       id="{{$locale}}[name]"
                                       name="{{$locale}}[name]"
                                       v-validate="'required'"
                                       value="{{ old($locale)['name'] ?? ($category->translate($locale)['name'] ?? '') }}"
                                       data-vv-as="&quot;{{ __('blog::app.blog.name') }}&quot;">
                            ...
                            <div class="control-group" :class="[errors.has('channels[]') ? 'has-error' : '']">
                                ...
                                <?php $selectedOptionIds = old('inventory_sources') ?: $category->channels->pluck('id')->toArray() ?>
                        ...
                        <div slot="body">
                            <div class="control-group" :class="[errors.has('{{$locale}}[url_key]') ? 'has-error' : '']">
                                ...
                                <input type="text" class="control" name="{{$locale}}[url_key]" v-validate="'required'"
                                       value="{{ old($locale)['url_key'] ?? ($category->translate($locale)['url_key'] ?? '') }}"
