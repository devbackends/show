...
<accordian :title="'{{ __('admin::app.catalog.products.variations') }}'" :active="false">
    <div slot="body">
        ...
        <button type="button" class="btn btn-md theme-btn" @click="showModal('addVariant')">
...
@push('scripts')
    ...
    <script type="text/x-template" id="variant-form-template">
        <form class="account-table-content" method="POST" action="{{ route('admin.catalog.products.store') }}" data-vv-scope="add-variant-form" @submit.prevent="addVariant('add-variant-form')">

            <div class="page-content">
                <div class="form-container">

                    <div v-for='(attribute, index) in super_attributes' class="form-group control-group" :class="[errors.has('add-variant-form.' + attribute.code) ? 'has-error' : '']">
    ...
    <script type="text/x-template" id="variant-item-template">
        <tr>
            <td>
                <input type="hidden" :name="[variantInputName + '[vendor_id]']" value="{{ $productFlat->marketplace_seller_id }}"/>

                <div class="form-group"  :class="[errors.has(variantInputName + '[sku]') ? 'has-error' : '']">
            ...
            <td>
                <div class="form-group" :class="[errors.has(variantInputName + '[name]') ? 'has-error' : '']">
            ...
            <td v-for='(attribute, index) in superAttributes'>
                <div class="form-group" >
                    <input class="form-style" type="hidden" :name="[variantInputName + '[' + attribute.code + ']']" :value="variant[attribute.code]"/>
                    ...
                    <div class="dropdown-container">
                        <ul type="none">
                            <li v-for='(inventorySource, index) in inventorySources'>
                                <div class="form-group" :class="[errors.has(variantInputName + '[inventories]sss[' + inventorySource.id + ']') ? 'has-error' : '']">
            ...
            <td class="actions">
                <a :href="['{{ route('marketplace.account.products.index') }}/edit/' + variant.id]">
