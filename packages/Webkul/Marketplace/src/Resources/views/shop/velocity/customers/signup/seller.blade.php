...
@push('scripts')
    <script type="text/x-template" id="seller-controls-template">
        ...
        <div class="seller-form-controls">
            ...
            <div class="form-group" :class="[errors.has('url') ? 'has-error' : '']" v-if="want_to_be_seller == 1">
                <label for="url" class="required">{{ __('marketplace::app.shop.sellers.account.signup.shop_url') }}</label>

                <input type="text" class="control form-style" name="url" v-validate="'required'" value="{{ old('url') }}" data-vv-as="&quot;{{ __('marketplace::app.shop.sellers.account.signup.shop_url') }}&quot;" v-on:keyup="checkShopUrl($event.target.value)">
