<select v-validate="'{{$validations}}'" class="custom-select" id="{{ $attribute->code }}" name="{{ $attribute->code }}" data-vv-as="&quot;{{ $attribute->admin_name }}&quot;">
                ...
                <div>
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form   method="POST" action="{{ route('admin.catalog.attributes.option.store') }}" id="add-attribute-form{{$attribute->id}}" data-vv-scope="add-attribute-form{{$attribute->id}}" @submit.prevent="addAttributeOption('add-attribute-form')" class="">
                                    @csrf
                                    <div class="form-group mb-0"
                                         :class="[errors.has('variant_option') ? 'has-error' : '']">
                                        <label for="variant_option">Add {{ $attribute->name }} Option</label>
                                        <input type="text" required v-model="variant_option" id="variant_option"
                                               class="form-control" name="variant_option">
                    ...
                    methods: {
                        addAttributeOption: function (formScope,e) {
                            ...
                            this.$validator.validateAll(formScope).then((result) => {
                                if (result) {
                                    var this_this = this;
                                    this_this.$http.post("{{ route('admin.catalog.attributes.option.store') }}",
                                    {'variant_option':this_this.variant_option,'attribute_id':this_this.attribute_id})
                                        .then(response => {
                                            if(response.status==200){
                                                this_this.variant_option=null;
                                                if(response.data.status=='success'){
                                                    $('#{{ $attribute->code }}').append(`<option value="${response.data.id}">${response.data.option}</option>`);
                                                    $('#{{ $attribute->code }}').val(response.data.id);
                                                }
