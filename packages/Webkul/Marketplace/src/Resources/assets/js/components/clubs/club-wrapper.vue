<template>
    <div>
        ...
        <div v-if="items.length > 0" class="shows-list__items row mx-n1 pb-4">
            <div class="shows-list__item col-12 col-md-6 col-lg-4 px-1 mb-2" v-for='(club, index) in items' :key="index">
                <club-item :clubItem="club"></club-item>
            </div>
    ...
    export default {
        ...
        methods: {
            showMore(filters) {
                ...
                this.$http.get(
                    `${this.$root.baseUrl}/marketplace/clubs/api?${params}`
                ).then(response => {
