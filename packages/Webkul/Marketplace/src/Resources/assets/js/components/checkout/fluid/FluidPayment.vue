    ...
    <div>
        <div class="cards">
                <div class="custom-control custom-radio" :key="index" v-for="(card, index) in cards">
                    <input type="radio" id="card" name="card" :value="index" class="custom-control-input" v-model="selectedCard">
                    <label class="custom-control-label" for="card">{{getCardFullName(card)}}<a v-on:click="removeCard(index)" href="javascript:;"><i class="far fa-times ml-2"></i></a></label>
                </div>
        </div>
        <button data-toggle="modal"
                data-target="#newFluidCardPopup" id="add-fluid-credit-card"
...
export default {
    ...
    async mounted() {
        ...
        if(this.selectedPayment=='fluid' && !this.selectedCard && this.cards.length > 0){
            this.selectedCard=0;
        }
    },
    methods: {
        ...
        removeCard(index){
         if(typeof this.cards[index].id != 'undefined'){
             this.$http.delete(API_ENDPOINTS.fluid.card.remove.replace('{cardId}', this.cards[index].id )).then((response) => {
