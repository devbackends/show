<template>
    <div>
        ...
        <div v-if="items.length > 0" class="shows-list__items row mx-n1 pb-4">
            <div class="shows-list__item col-12 col-md-6 col-lg-4 px-1 mb-2" v-for='(gunRange, index) in items' :key="index">
                <gun-range-item :gunRangeItem="gunRange"></gun-range-item>
            </div>
    ...
    export default {
        ...
        methods: {
            showMore(filters) {
                ...
                this.$http.get(
                    `${this.$root.baseUrl}/marketplace/gun-ranges/api?${params}`
                ).then(response => {
