<template>
    <div>
        <div class="shows-list__filter">
            ...
            <div class="form-group ffl-list__filter-miles">
                <label>Miles</label>
            ...
            <div class="form-group">
                <button type="submit" @click="showMore(true)" class="btn btn-outline-gray">Apply filter</button>
            </div>
            <span class="info ml-md-auto">
                <span class="ffl-preferred__legend rounded-circle"></span>
                Preferred FFL
            </span>
        </div>
        <div v-if="items.length > 0" class="shows-list__items row mx-n1 mb-4">
            <div class="shows-list__item col-12 col-md-6 col-lg-4 px-1 mb-2" v-for='(ffl, index) in items' :key="index">
                <ffl-item :fflItem="ffl" :isPreferred="ffl.is_approved==1 && ffl.source=='on_boarding_form'"></ffl-item>
            </div>
        ...
        <div v-else-if="items.length === 0 && !loading" class="shows-list__items row mx-n1 mb-4">
    ...
    export default {
        ...
        methods: {
            showMore(filters) {
                ...
                this.$http.get(
                    `${this.$root.baseUrl}/marketplace/ffls/api?${params}`
                ).then(response => {
