<template>
    <div>
        ...
        <div v-if="items.length > 0" class="shows-list__items row mx-n1 mb-4">
            <div class="col-12 col-md-6 col-lg-4 px-1 mb-2" v-for='(instructor, index) in items'
                    :key="index">
                <instructor-item :instructorItem="instructor"></instructor-item>
            </div>
    ...
    export default {
        ...
        methods: {
            showMore(filters) {
                ...
                this.$http.get(
                    `${this.$root.baseUrl}/marketplace/instructors/api?${params}`
                ).then(response => {
