export const FIREARM_FAMILY = 'firearm'
export const GOOGLE_MAPS_API_KEY = "AIzaSyDnUKjS6BAjGhL8XL8SacAYmDQMpWEchXs"
export const FFL_PREFERRED_SOURCE = "on_boarding_form"
export const GOOGLE_GEOCODE_API_URL = "https://maps.googleapis.com/maps/api/geocode/json";
export const API_ENDPOINTS = {
    getShippingRates: 'api/checkout/shipping-rates',
    getCart: 'api/checkout/cart',
    getAddresses: "api/addresses",
    getClosestFfls: "api/ffl/find/closest",
    getFflByZip: "api/ffl/find",
    getStates: "api/country-states",
    getCustomerInfo: "api/customer/get",
    postAddress: "api/addresses/create",
    getPaymentMethods: "api/checkout/payment/{sellerId}",
    postCheckoutSaveBillingAddress: "api/checkout/save-billing-address",
    postCheckoutShippingAddress: "api/checkout/save-shipping-address",
    postCheckoutSaveShippingRate: "api/checkout/save-shipping",
    postCheckoutSavePaymentMethod: "api/checkout/save-payment",
    postCheckoutSaveFflShippingAddress: "api/checkout/save-ffl-address",
    postCheckoutSaveFflShippingRate: "api/checkout/save-ffl-shipping",
    postCheckoutSaveOrder: "api/checkout/save-order",
    getFflById: "api/ffl/find",
    getConfig: "api/config",
    getCompany: "api/company",
    getCompanyAddress: "api/company-addresses",
    getAllProducts: "getAllProducts",
    checkConfigurableProduct: "checkConfigurableProduct",
    fluid: {
        getTokenizerinfo: "/fluid/api/get-tokenizer-info/{sellerId}",
        card: {
            get:   "/fluid/api/card/{customerId}/{sellerId}",
            remove:"/fluid/api/card/{cardId}"
        }
    },
    stripe: {
        get_mode: "/checkout/get-stripe-mode",
        get_token: "/checkout/sendtoken/{cartId}",
        card: {
            get: "/checkout/stripe/get-cards"
        }
    },
    authorize: {
        card: {
            get: "/checkout/authorize/get-cards"
        }
    },
    bluedog: {
        card: {
            remove:"/fluid/api/card/{cardId}"
        }
    },
    cart: {
        get: "/marketplace/cart",
        add: "/marketplace/cart",
        update: "/marketplace/cart/update",
        remove: "/marketplace/cart/{sellerId}/{itemId}",
    },
    sellerOnboarding: {
        // Get endpoints
        getShippingMethods: "/marketplace/account/onboarding/shipping-methods",
        getPlans: "/marketplace/account/onboarding/plan",
        getTokenizerInfo: "/marketplace/account/onboarding/tokenizer-info",

        // Post endpoints
        storeShopInfo: "/marketplace/account/onboarding/store/shop-info",
        storeShippingInfo: "/marketplace/account/onboarding/store/shipping-info",
        storePlan: "/marketplace/account/onboarding/store/plan",
        storePayments: "/marketplace/account/onboarding/store/payments",
        storeSeller: "/marketplace/account/onboarding/store",
    },
    checkout: "/checkout/onepage/{sellerId}",
};