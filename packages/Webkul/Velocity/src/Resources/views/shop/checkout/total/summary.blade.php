<div class="order-summary fs16">
    ...
    <div class="row">
        <span class="paragraph col-8">{{ __('velocity::app.checkout.sub-total') }}</span>
        <span class="paragraph col-4 text-right">{{ core()->currency($cart->base_sub_total) }}</span>
    </div>

    @if ($cart->selected_shipping_rate)
        <div class="row">
            <span class="paragraph col-8">{{ __('shop::app.checkout.total.delivery-charges') }}</span>
            <span class="paragraph col-4 text-right">{{ core()->currency($cart->selected_shipping_rate->base_price) }}</span>
        </div>
    @endif
