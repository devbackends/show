...
<div class="layered-filter-wrapper col-lg-2" style="margin-bottom:0;">
...
@push('scripts')
    <script type="text/x-template" id="layered-navigation-template">
        <div v-if="attributes.length > 0" class="side-filter__title">
    ...
    <script type="text/x-template" id="filter-attribute-item-template">
        <div :class="`side-filter__subtitle cursor-pointer filter-attributes-item ${active ? 'active' : ''}`">
            <div class="filter-attributes-title d-flex" @click="active = !active">
                <h4>@{{ attribute.name ? attribute.name : attribute.admin_name }}</h4>

                <div class="ml-auto side-filter__subtitle-right">
                    <span class="side-filter__clear-btn" v-if="appliedFilters.length" @click.stop="clearFilters()">
                        {{ __('shop::app.products.remove-filter-link-title') }}
                    </span>

                    <i :class="`fas ${active ? 'fa-chevron-up' : 'fa-chevron-down'}`"></i>
                </div>
            </div>

            <div class="filter-attributes-content">
                <ul type="none" v-if="attribute.type != 'price'">
                    <li
                        class="item"
                        v-for='(option, index) in attribute.options'>

                        <div class="radio-button checkbox-button" @click="optionClicked(option.id, $event)">
                            ...
                            <div class="radio-button__label">@{{ option.label ? option.label : option.admin_name }}</div>
                        </div>
