...
Route::group(['prefix' => 'api'], function ($router) {
    ...
    Route::group(['namespace' => 'Webkul\API\Http\Controllers\Shop', 'middleware' => ['locale', 'theme', 'currency']], function ($router) {
        ...
        Route::group(['prefix' => 'marketplace'], function (){
            Route::get('products', 'Marketplace\ProductController@index');
        });
        ...
        Route::get('get-product-by-sku/{sku}', 'ProductController@getProductBySku');
        ...
        Route::post('add-product', 'ProductController@store')->name('api.products.add');

        Route::put('edit-product/{id}', 'ProductController@update')->defaults('_config', [
            'authorization_required' => true,
        ])->name('api.products.edit');

        Route::post('add-product-images', 'ProductController@addProductImage');

        Route::post('remove-product-image', 'ProductController@removeProducImage');

        Route::get('product-inventory/{id}', 'ProductController@getProductInventory')->defaults('_config', [
            'authorization_required' => true,
        ])->name('api.products.inventory.get');

        Route::put('product-inventory/{id}', 'ProductController@updateProductInventory')->defaults('_config', [
            'authorization_required' => true,
        ])->name('api.products.inventory.get');

        Route::get('send-webhook', 'ProductController@Send')->name('send-webhook');
        ...
        Route::get('customer/logout', 'SessionController@destroy')->middleware('auth:api')->name('api.customers.logout');

        Route::get('customer/get', 'SessionController@get')->middleware('auth:api')->name('api.customers.get-logged-profile');
        ...
        ])->middleware('auth:api')->name('api.customers.get');
        ...
        Route::get('company-addresses', 'AddressController@getCompany')->defaults('_config', [
            'authorization_required' => true,
        ]);
        ...
        Route::group(['prefix' => 'checkout'], function ($router) {
            ...
            Route::get('payment/{sellerId}', 'CheckoutController@getPayment');
