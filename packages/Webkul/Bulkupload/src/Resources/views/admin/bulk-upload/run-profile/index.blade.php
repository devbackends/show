...
@push('scripts')
        ...
        <div class="run-profile">
            <form action="{{ route('bulk-upload-admin.run-profile') }}" method="post">
                    ...
                    <div class="control-group">
                        ...
                        <label>Sellect Seller</label>
                        @php
                            $sellers = app('Webkul\Marketplace\Repositories\SellerRepository')->all();
                        @endphp
                        <select class="control mb-3" id="seller"  v-model="seller" name="seller">
                            <option>Please Select</option>
                                @foreach ($sellers as $seller)
                                    @if($seller->shop_title && $seller->is_approved)
                        ...
                        <select class="control mb-3" id="data-flow-profile"  v-model="data_flow_profile" name="data_flow_profile">
                            <option>Please Select</option>
                                @if (isset($profiles))
                        ...
                        <div class="page-action">
                            <button type="submit" :class="{ disabled: isDisabled }" :disabled="isDisabled" @click.prevent="runProfiler" class="btn btn-primary">
                                Run
                            </button>
                        </div>
                    </div>
    ...
    <script>
        Vue.component('profiler', {
            ...
            computed: {
                isDisabled () {
                    if (this.data_flow_profile == '' || this.data_flow_profile == 'Please Select' || this.seller == '' || this.seller == 'Please Select') {
                        return true;
                    } else {
                        return false;
                    }
