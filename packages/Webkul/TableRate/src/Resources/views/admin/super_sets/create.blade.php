...
@section('page_title')
    {{ __('tablerate::app.admin.supersets.add-title') }}
@stop

@section('content')
    <div class="content">
        <form method="POST" action="{{ route('admin.tablerate.supersets.store') }}" @submit.prevent="onSubmit" enctype="multipart/form-data">
            @csrf
                    ...
                    <accordian :title="'{{ __('tablerate::app.admin.supersets.general') }}'" :active="true">
                        <div slot="body">
                            ...
                            <div class="control-group" :class="[errors.has('code') ? 'has-error' : '']">
                                ...
                                <input type="text" v-validate="'required'" class="control" name="code" value="{{ old('code') }}" data-vv-as="&quot;{{ __('tablerate::app.admin.supersets.code') }}&quot;">
                                <span class="control-error" v-if="errors.has('code')">@{{ errors.first('code') }}</span>
                            </div>

                            <div class="control-group" :class="[errors.has('name') ? 'has-error' : '']">
                                <label for="text" class="required">{{ __('tablerate::app.admin.supersets.name') }}<i class="export-icon"></i> </label>
                                <input type="text" v-validate="'required'" class="control" name="name" value="{{ old('name') }}" data-vv-as="&quot;{{ __('tablerate::app.admin.supersets.name') }}&quot;">
