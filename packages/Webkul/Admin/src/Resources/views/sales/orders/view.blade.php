...
@section('content-wrapper')
    ...
    <div class="content full-page">
        ...
        <div class="page-header">
            ...
            <div class="page-action">
                @if ($order->canCancel())
                    <a href="{{ route('admin.sales.orders.cancel', $order->id) }}" class="btn btn-primary"
                ...
                @if ($order->canInvoice())
                    <a href="{{ route('admin.sales.invoices.create', $order->id) }}" class="btn btn-primary">
                        {{ __('admin::app.sales.orders.invoice-btn-title') }}
                    </a>
                @endif

                @if ($order->canRefund())
                    <a href="{{ route('admin.sales.refunds.create', $order->id) }}" class="btn btn-primary">
                        {{ __('admin::app.sales.orders.refund-btn-title') }}
                    </a>
                @endif

                @if ($order->canShip())
                    <a href="{{ route('admin.sales.shipments.create', $order->id) }}" class="btn btn-primary">
                    ...
                    <div class="sale-container">
                        ...
                        <accordian :title="'{{ __('admin::app.sales.orders.address') }}'" :active="true">
                            <div slot="body">
                                ...
                                @if($order->ffl_address)
                                    <div class="sale-section">
                                        <div class="secton-title">
                                            <span>FFL Address</span>
                                        </div>

                                        <div class="section-content">

                                            @include ('admin::sales.address', ['address' => $order->ffl_address])
                        ...
                        <accordian :title="'{{ __('admin::app.sales.orders.payment-and-shipping') }}'" :active="true">
                            <div slot="body">
                                ...
                                @if ($order->shipping_address || !empty($ffl))
                                    <div class="sale-section">
                                        <div class="secton-title">
                                        ...
                                        <div class="section-content">
                                            @if($order->shipping_address)
                                                <div class="row">
                                                    <span
                                                        class="title">{{ __('admin::app.sales.orders.shipping-method') }}</span>
                                                    <span class="value">{{ $order->shipping_title }}</span>
                                                </div>

                                                <div class="row">
                                                    <span
                                                        class="title">{{ __('admin::app.sales.orders.shipping-price') }}</span>
                                                    <span
                                                        class="value">{{ core()->formatBasePrice($order->base_shipping_amount - (!empty($ffl) ? $ffl->base_price : 0)) }}</span>
                                                </div>
                                            @endif
                                            ...
                                            @if(!empty($ffl))
                                                <div class="row">
                                                    <span
                                                        class="title">Ffl {{ __('admin::app.sales.orders.shipping-method') }}</span>
                                                    <span class="value">{{ $ffl->method_title }}</span>
                                                </div>
                                                <div class="row">
                                                    <span
                                                        class="title">{{ __('admin::app.sales.orders.shipping-price') }}</span>
                                                    <span
                                                        class="value">{{ core()->formatBasePrice($ffl->base_price) }}</span>
                                                </div>
                                            @endif
                                        </div>
                                    </div>
    ...
    @push('scripts')
        ...
        <script type="text/x-template" id="notes-form-template">
            <div id="form-app" class="sale-container">
                <form method="POST" data-vv-scope="notes-form"
                      @submit.prevent="check_form('notes-form')">
                    <div class="form-group" :class="[errors.has('notes') ? 'has-error' : '']">
                <textarea style="min-height:200px;" class="form-control" placeholder="Notes"
                          v-model="order.notes"
                          name="notes"
                          data-vv-as="&quot;Notes&quot;"></textarea>
        ...
        <script>
            Vue.component('notes-form', {
                ...
                template: '#notes-form-template',
                methods: {
                    check_form: function (formScope) {
                        var this_this = this;
                        this.$validator.validateAll(formScope).then((result) => {
                            if (result) {
                                this.$http.post("{{ route('save.order.notes',$order->id) }}", {'order_notes': this.order.notes})
