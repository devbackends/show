        ...
        <form method="POST" action="{{ route('admin.catalog.attributes.update', $attribute->id) }}" @submit.prevent="onSubmit" enctype="multipart/form-data">
            ...
            <div class="page-content">
                <div class="form-container">
                    ...
                    <accordian :title="'{{ __('admin::app.catalog.attributes.general') }}'" :active="true">
                        <div slot="body">
                            ...
                            <div class="control-group">
                                ...
                                <select class="control js-example-basic-multiple" id="type" disabled="disabled">
                    ...
                    <accordian :title="'{{ __('admin::app.catalog.attributes.label') }}'" :active="true">
                        <div slot="body">
                            ...
                            <div class="control-group" :class="[errors.has('description') ? 'has-error' : '']">
                                ...
                                <input type="text" class="control" id="description" name="description" value="{{ old('description') ?: $attribute->description }}" data-vv-as="&quot;Description&quot;"/>
                                <span class="control-error" v-if="errors.has('description')">@{{ errors.first('description') }}</span>
                            </div>

                            <div class="control-group">
                                <label for="primary_filter">Primary Filter</label>
                                <select class="control" id="primary_filter" name="primary_filter">
                                    <option value="0" {{ $attribute->primary_filter ? '' : 'selected' }}>{{ __('admin::app.catalog.attributes.no') }}</option>
                                    <option value="1" {{ $attribute->primary_filter ? 'selected' : '' }}>{{ __('admin::app.catalog.attributes.yes') }}</option>
                                </select>
                            </div>
                    ...
                    <accordian :id="'{{ __('admin::app.catalog.attributes.configuration') }}'" :title="'{{ __('admin::app.catalog.attributes.configuration') }}'" :active="true">
                        <div slot="body">
...
@push('scripts')
    ...
    <script type="text/x-template" id="options-template">
        <div>
                        ...
                        <tr v-for="(row, index) in optionRows">
                            ...
                            <td>
                                <div class="control-group" :class="[errors.has(sortOrderName(row)) ? 'has-error' : '']">
                                    <input readonly type="text" v-validate="'required|numeric'" v-model="row['sort_order']" :name="sortOrderName(row)" class="control sort-order-input" data-vv-as="&quot;{{ __('admin::app.catalog.attributes.position') }}&quot;"/>
            ...
            <button type="button" class="btn btn-primary" id="add-option-btn" style="margin-top: 20px" @click="addOptionRow()">
    ...
    <script>
        Vue.component('option-wrapper', {
            ...
            created: function () {
                ...
                this.optionRows = this.optionRows.sort((a, b) => a.sort_order - b.sort_order);

                let optionsToUpdate = [];
                let order = 0;
                this.optionRows.map(val => {
                    if (val.sort_order == order) {
                        order++;
                        return val;
                    }
                    val.sort_order = order
                ...
                if (optionsToUpdate.length > 0) {
                    this.updateOptionsSortOrderInDB(optionsToUpdate);
                }
            ...
            mounted() {
                eventBus.$on('updateSortOrder', payload => {

                    let sortOrdersToUpdate = [];

                    // Set current sort order and new
                    const newSortOrder = parseInt(payload.newSortOrder);
                    const currentSortOrder = parseInt(payload.el.querySelector('.sort-order-input').value);

                    let options = [...this.optionRows];

                    options.forEach(el => {

                        // Filter items that don't need to be changed
                        if (newSortOrder < currentSortOrder) {
                            if (el.sort_order > currentSortOrder || el.sort_order < newSortOrder) return true;
                        } else {
                            if (el.sort_order < currentSortOrder || el.sort_order > newSortOrder) return true;
                        }

                        // Change sort order of moved item
                        if (parseInt(el.sort_order) === currentSortOrder) {
                            sortOrdersToUpdate.push({
                                id: el.id,
                                sort_order: newSortOrder,
                            });
                            return el.sort_order = newSortOrder;
                        }

                        // Change sort order of affected items
                        if (newSortOrder < currentSortOrder) {
                            sortOrdersToUpdate.push({
                                id: el.id,
                                sort_order: parseInt(el.sort_order) + 1,
                            });
                            return el.sort_order = parseInt(el.sort_order) + 1;
                        } else {
                            sortOrdersToUpdate.push({
                                id: el.id,
                                sort_order: parseInt(el.sort_order) - 1,
                    ...
                    this.optionRows = options;

                    // Update sort order in DB via server request
                    this.updateOptionsSortOrderInDB(sortOrdersToUpdate);
            ...
            methods: {
                updateOptionsSortOrderInDB(sortOrdersToUpdate) {
                    $.ajax({
                        type: 'put',
                        url: "<?= route('admin.catalog.attributes.updateOptionSortOrder', $attribute->id) ?>",
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
                        data: {
                            data: JSON.stringify(sortOrdersToUpdate),
                        }
                    });
                },
                ...
                addOptionRow: function (isNullOptionRow) {
                    ...
                    let row = {'id': id};
                    row.sort_order = parseInt(Object.keys(this.optionRows).pop()) + 1;
