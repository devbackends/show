<?php
use Illuminate\Support\Facades\Storage;$validations = [];
    $disabled = false;
    ...
    @else
        ...
        <div class="control-group {{ $field['type'] }}" @if ($field['type'] == 'multiselect') :class="[errors.has('{{ $firstField }}[{{ $secondField }}][{{ $thirdField }}][{{ $field['name'] }}][]') ? 'has-error' : '']" @else :class="[errors.has('{{ $firstField }}[{{ $secondField }}][{{ $thirdField }}][{{ $field['name'] }}]') ? 'has-error' : '']" @endif>
            ...
            @elseif ($field['type'] == 'textarea')
                ...
                <textarea v-validate="'{{ $validations }}'" @if(isset($field['placeholder']))  placeholder="{{$field['placeholder']}}" @endif  class="control" id="{{ $firstField }}[{{ $secondField }}][{{ $thirdField }}][{{ $field['name'] }}]" name="{{ $firstField }}[{{ $secondField }}][{{ $thirdField }}][{{ $field['name'] }}]" data-vv-as="&quot;{{ trans($field['title']) }}&quot;">{{ old($name) ?: core()->getConfigData($name) }}</textarea>
                @if(isset($field['note']))  <label class="note-label">{{$field['note']}}</label> @endif
            ...
            @elseif ($field['type'] == 'image')
                ...
                <?php
                    try {
                        ...
                        $src = Storage::disk('public')->url(core()->getConfigData($name));
                    }
