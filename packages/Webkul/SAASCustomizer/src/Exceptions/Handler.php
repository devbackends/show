...
class Handler extends ExceptionHandler
{
    ...
    public function render($request, Throwable $exception)
    {
        ...
        if ($exception->getMessage() == 'company_blocked_by_administrator') {
            return $this->response($path, 404, trans('saas::app.admin.tenant.exceptions.company-blocked-by-administrator'), 'company_blocked_by_administrator');
        }

        if ($exception->getMessage() == 'not_allowed_to_visit_this_section') {
            return $this->response($path, 400, trans('saas::app.admin.tenant.exceptions.not-allowed-to-visit-this-section'), 'not_allowed_to_visit_this_section');
        }

        if ($exception->getMessage() == 'illegal_action') {
            return $this->response($path, 400, trans('saas::app.admin.tenant.exceptions.illegal-action'), 'illegal_action');
        }

        if ($exception->getMessage() == 'invalid_admin_login' || $exception->getMessage() == 'invalid_customer_login') {
            return $this->response($path, 401, trans('saas::app.admin.tenant.exceptions.auth'));
        }
