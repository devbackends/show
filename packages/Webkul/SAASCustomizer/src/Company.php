...
class Company
{
    ...
    public function getCurrent()
    {
        ...
        if ($currentURL == $primaryServerNameWithoutProtocol) {
            if(request()->is('super/*')){
                $this->company = 'super-company';
                return $this->company;
            }else{
                $this->company = $this->companyRepository->findWhere(['email' => 'justin@devvly.com'])->first();
                return $this->company;
            }

        } else {
            $this->company = $this->companyRepository->findWhere(['domain' => $currentURL]);

            if ($this->company->isEmpty()) {
                throw new \Exception('domain_not_found', 400);
            } else {
                $this->company = $this->company->first();

                if ($this->company->is_active == 0) {
