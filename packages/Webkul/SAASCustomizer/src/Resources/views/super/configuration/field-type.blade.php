...
?>
    ...
    @if ($field['type'] == 'depends')
        ...
        <?php
            ...
            $selectedOption = company()->getSuperConfigData($name) ?? '';
        ?>
    ...
    @else
        ...
        <div class="control-group {{ $field['type'] }}" @if ($field['type'] == 'multiselect') :class="[errors.has('{{ $firstField }}[{{ $secondField }}][{{ $thirdField }}][{{ $field['name'] }}][]') ? 'has-error' : '']" @else :class="[errors.has('{{ $firstField }}[{{ $secondField }}][{{ $thirdField }}][{{ $field['name'] }}]') ? 'has-error' : '']" @endif>
            ...
            @elseif ($field['type'] == 'color')
                ...
                <input type="color" v-validate="'{{ $validations }}'" class="control" id="{{ $firstField }}[{{ $secondField }}][{{ $thirdField }}][{{ $field['name'] }}]" name="{{ $firstField }}[{{ $secondField }}][{{ $thirdField }}][{{ $field['name'] }}]" value="{{ old($name) ?: company()->getSuperConfigData($name) }}" data-vv-as="&quot;{{ trans($field['title']) }}&quot;">


            @elseif ($field['type'] == 'textarea')

                <textarea v-validate="'{{ $validations }}'" class="control" id="{{ $firstField }}[{{ $secondField }}][{{ $thirdField }}][{{ $field['name'] }}]" name="{{ $firstField }}[{{ $secondField }}][{{ $thirdField }}][{{ $field['name'] }}]" data-vv-as="&quot;{{ trans($field['title']) }}&quot;">{{ old($name) ?: company()->getSuperConfigData($name) }}</textarea>
            ...
            @elseif ($field['type'] == 'country')
                ...
                <?php
                    $countryCode = company()->getSuperConfigData($name) ?? '';
                ?>
            ...
            @elseif ($field['type'] == 'state')
                ...
                <?php
                    $stateCode = company()->getSuperConfigData($name) ?? '';
                ?>
            ...
            @elseif ($field['type'] == 'image')
                ...
                <?php
                try {
                    $src = Storage::url(company()->getSuperConfigData($name));
                } catch (\Exception $error) {
                    $src = Storage::disk('public')->url(company()->getSuperConfigData($name));
                }
                $result = company()->getSuperConfigData($name);
                ?>
                ...
                <input type="file" v-validate="'{{ $validations }}'" class="control" id="{{ $firstField }}[{{ $secondField }}][{{ $thirdField }}][{{ $field['name'] }}]" name="{{ $firstField }}[{{ $secondField }}][{{ $thirdField }}][{{ $field['name'] }}]" value="{{ old($name) ?: company()->getSuperConfigData($name) }}" data-vv-as="&quot;{{ trans($field['title']) }}&quot;" style="padding-top: 5px;">
