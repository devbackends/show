...
@section('content')
    <div class="content">
        <form method="POST" action="{{ route('super.agents.create') }}" @submit.prevent="onSubmit" enctype="multipart/form-data">
            ...
            <div class="page-content">
                <div class="container">
                    ...
                    <accordian :title="'{{ __('saas::app.super-user.settings.locales.general') }}'" :active="true">
                        <div slot="body">
                            ...
                            <div class="control-group" :class="[errors.has('last_name') ? 'has-error' : '']">
                                ...
                                <input type="text" v-validate="'required'" class="control" id="last_name" name="last_name" data-vv-as="&quot;{{ __('saas::app.super-user.settings.agents.last-name') }}&quot;" value="{{ old('last_name') }}" />
                                <span class="control-error" v-if="errors.has('last_name')">@{{ errors.first('last_name') }}</span>
                            </div>

                            <div class="control-group" :class="[errors.has('email') ? 'has-error' : '']">
                                <label for="email" class="required">{{ __('saas::app.super-user.settings.agents.email') }}</label>
                                <input type="text" v-validate="'required|email'" class="control" id="email" name="email" data-vv-as="&quot;{{ __('saas::app.super-user.settings.agents.email') }}&quot;" value="{{ old('email') }}" />
                    ...
                    {!! view_render_event('bagisto.super.settings.agent.create.after') !!}
