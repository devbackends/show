...
@section('page_title')
    {{ __('saas::app.super-user.settings.exchange-rates.edit-title') }}
@stop

@section('content')
    <div class="content">

        <form method="POST" action="{{ route('super.exchange_rates.update', $exchangeRate->id) }}" @submit.prevent="onSubmit">
                ...
                <div class="form-container">
                    ...
                    <div class="table">
                        <table>
                            ...
                            <tbody>
                                <tr>
                                    {!! view_render_event('bagisto.super.settings.exchangerate.edit.before') !!}
                                    ...
                                    <td>
                                        <div class="control-group" :class="[errors.has('rate') ? 'has-error' : '']">
                                            <input v-validate="'required'" class="control" id="rate" name="rate" data-vv-as="&quot;{{ __('saas::app.super-user.settings.exchange-rates.rate') }}&quot;" value="{{ old('rate') ?: $exchangeRate->rate }}"/>
                                            <span class="control-error" v-if="errors.has('rate')">@{{ errors.first('rate') }}</span>
                                        </div>
                                    </td>

                                    {!! view_render_event('bagisto.super.settings.exchangerate.edit.after') !!}
                                <tr>
                            </tbody>
                        </table>
                    </div>
