...
class Bundle extends AbstractType
{
    ...
    */
    ...
    {
        $pricesComps = $this->getPriceComponents($seller_id);
        return $pricesComps['html'];
    }
    ...
    public function getPriceComponents($seller_id = 0)
    {
        ...
        if ($prices['from']['regular_price']['price'] != $prices['from']['final_price']['price']) {
            ...
            $price = $prices['from']['final_price']['formated_price'];
        } else {
            $priceHtml .= '<span>' . $prices['from']['regular_price']['formated_price'] . '</span>';
            $price = $prices['from']['regular_price']['formated_price'];
        }
        ...
        if ($prices['from']['regular_price']['price'] != $prices['to']['regular_price']['price']
            ...
            if ($prices['to']['regular_price']['price'] != $prices['to']['final_price']['price']) {
                ...
                $price = $prices['to']['final_price']['formated_price'];
            } else {
                $priceHtml .= '<span>' . $prices['to']['regular_price']['formated_price'] . '</span>';
                $price = $prices['to']['regular_price']['formated_price'];
            }
        }
     ...
     * @return array
     */
    public function prepareForCart($data,$seller_id=0)
    {
    ...
    public function validateCartItem($item, $sellerId = 0)
    {
        ...
        foreach ($item->children as $childItem) {
            $childItem->product->getTypeInstance()->validateCartItem($childItem, $sellerId);
