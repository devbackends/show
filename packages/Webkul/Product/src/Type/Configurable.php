...
class Configurable extends AbstractType
{
    ...
    public function createVariant($product, $permutation, $data = [])
    {
        ...
        dd('add product inventory here createVariant');
...
/*                'channel' => $attribute->value_per_channel ? $data['channel'] : null,
     ...
     * @return bool
     */
    public function isItemHaveQuantity($cartItem, $selerId = 0)
    {
        return $cartItem->child->product->getTypeInstance()->haveSufficientQuantity($cartItem->quantity, $selerId);
    }
    ...
    {
        $comps = $this->getPriceComponents($seller_id);
        return $comps['html'];
    }
    ...
    public function getPriceComponents($seller_id = 0)
    {
        ...
        $html = '<span class="price-label">' . trans('shop::app.products.price-label') . '</span>'
            . '<span class="final-price">' . core()->currency($price) . '</span>';
        $comps = [
            'price' => $price,
            'html' => $html,
        ];
