        ...
        if (isset($data['attribute_groups'])) {
            foreach ($data['attribute_groups'] as $attributeGroupId => $attributeGroupInputs) {
                if (Str::contains($attributeGroupId, 'group_')) {
                    ...
                    if (isset($attributeGroupInputs['width'])) {
                        foreach ($attributeGroupInputs['width'] as $key => $attributeWidthInputs) {
                            $width=json_decode($attributeWidthInputs);
                            if($width->width){
                                $affected = DB::update(
                                    'update attribute_group_mappings set width='.$width->width.' where attribute_id ='.$width->attribute_id.' and attribute_group_id='.$width->attribute_group_id
                                );
