...
Route::group(['middleware' => ['web', 'locale', 'theme', 'currency']], function () {
    ...
    // Become a seller
    Route::get('/become-seller', 'Webkul\Shop\Http\Controllers\HomeController@becomeSeller')->defaults('_config', [
        'view' => 'shop::become-seller.index'
    ])->name('shop.become-seller.index');

    // Buy all page
    Route::get('/buyall', 'Webkul\Shop\Http\Controllers\ProductController@buyall')->defaults('_config', [
        'view' => 'shop::buyall.buyall'
    ])->name('shop.buyall');

    // Buy all page
    Route::get('/gun-giveaway', 'Webkul\Shop\Http\Controllers\HomeController@gunGiveAway')->defaults('_config', [
        'view' => 'shop::gun-giveaway.gun-giveaway'
    ])->name('shop.gun-giveaway');

    Route::get('/getAllProducts/{pagination}/{categoryId}', 'Webkul\Shop\Http\Controllers\ProductController@getAllProducts')->name('shop.getAllProducts');
    ...
    Route::get('/search-result/{type}/{search}', 'Webkul\Shop\Http\Controllers\SearchController@search')->name('shop.search.search-result');
    ...
    Route::group(['middleware' => ['checkout']], function(){
        Route::get('/checkout/onepage/{sellerId?}', 'Webkul\Shop\Http\Controllers\OnepageController@index')->defaults('_config', [
    ...
    // Update Product Review Form Store
    Route::post('/product/update-review', 'Webkul\Shop\Http\Controllers\ReviewController@UpdateReview')->defaults('_config', [
        'redirect' => 'customer.reviews.index'
    ])->name('shop.reviews.update-review');

    // Find product by SKU
    Route::get('/product/find/{sku}', 'Webkul\Shop\Http\Controllers\ProductController@findBySku')->name('shop.product.find.sku');
    ...
    Route::prefix('customer')->group(function () {
        ...
        Route::post('register', 'Webkul\Customer\Http\Controllers\RegistrationController@create')->name('customer.register.create');
    ...
    Route::get('/seller-product/{seller}/{slug}', 'Webkul\Shop\Http\Controllers\ProductsCategoriesProxyController@sellerProduct')->defaults('_config', [
        'view' => 'shop::products.view',
    ])->name('shop.sellerProduct.index');

    Route::get('/product/{slug}', 'Webkul\Shop\Http\Controllers\ProductsCategoriesProxyController@product')
        ->defaults('_config', [
            'view' => 'shop::products.view',
        ])->name('shop.product.index');

    Route::get('/category/{slug}/{secondarySlug?}/{thirdSlug?}', 'Webkul\Shop\Http\Controllers\ProductsCategoriesProxyController@category')
        ->defaults('_config', [
            'view' => 'shop::products.index',
        ])->name('shop.category.index');

    Route::post('/upload-customer-profile-picture', 'Webkul\Shop\Http\Controllers\HomeController@uploadCustomerProfilePicture')
           ->name('shop.customer.profile.upload-picture');

});
Route::group(['middleware' => ['web', 'locale', 'theme', 'currency']], function () {
    // Login Routes
    // Login form show
    Route::get('customer/login', 'Webkul\Customer\Http\Controllers\SessionController@show')->defaults('_config', [
        'view' => 'shop::customers.session.index',
    ])->name('customer.session.index');
});

Route::get('/getProductBaseImage/{slug}', 'Webkul\Product\Http\Controllers\ProductController@getProductBaseImage')
    ->name('product.get-base-image');
