...
class OrderDataGrid extends DataGrid
{
    ...
    public function prepareQueryBuilder()
    {
        $queryBuilder = DB::table('orders as order')
            ->addSelect('order.id', 'order.increment_id', 'order.status', 'order.created_at', 'order.grand_total', 'order.order_currency_code',DB::raw(" (select customer_id from marketplace_sellers where id=(ifnull((select marketplace_seller_id from marketplace_orders where marketplace_orders.order_id=order.id),0))) as customer_seller_id"))
        ...
        $this->addColumn([
            ...
            'wrapper'    => function ($value) {
                if ($value->status == 'processing') {
                    return '<span class="badge badge-md badge-primary">' . trans('shop::app.customer.account.order.index.processing') . '</span>';
