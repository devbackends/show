...
class CartRule
{
    ...
    public function collect()
    {
        ...
        foreach ($cart->items()->get() as $item) {
            $itemCartRuleIds = $this->process($item,$cart->seller_id);
     ...
     */
    public function getCartRules($seller_id=0)
    {
/*        static $cartRules;
        ...
        if (app(\Webkul\Checkout\Cart::class)->getCurrentCustomer()->check()) {
            $customerGroupId = app(\Webkul\Checkout\Cart::class)->getCurrentCustomer()->user()->customer_group_id;
        } else {
        ...
        $cartRules = $this->cartRuleRepository->scopeQuery(function ($query) use ($customerGroupId,$seller_id ) {
            return $query->leftJoin('cart_rule_customer_groups', 'cart_rules.id', '=', 'cart_rule_customer_groups.cart_rule_id')
                         ...
                         ->where('cart_rules.seller_id', $seller_id)
     ...
     * @return array
     */
    public function process(CartItem $item,$seller_id=0): array
    {
        ...
        foreach ($this->getCartRules($seller_id) as $rule) {
            ...
            switch ($rule->action_type) {
                ...
                case 'cart_fixed':
                    if(isset($this->itemTotals[$rule->id])){
    ...
    public function validateCouponCode()
    {
        ...
        if (! $coupon || ! in_array($coupon->cart_rule_id, explode(',', $cart->applied_cart_rule_ids))) {
            app(\Webkul\Checkout\Cart::class)->removeCouponCode();
        }
    }
