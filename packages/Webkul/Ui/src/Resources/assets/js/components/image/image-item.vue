<template>
    <label :data-count="imageCount" class="image-item" :for="_uid" v-bind:class="{ 'has-image': imageData.length > 0 }"
        ...
        <input type="file" :multiple="imageData === ''" v-validate="'mimes:image/*'" accept="image/*" :name="finalInputName" ref="imageInput" :id="_uid" @change="addImageView($event)" :required="required ? true : false" />
